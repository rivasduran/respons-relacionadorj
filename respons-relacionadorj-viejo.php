		...
		if(isset($_POST['relacionar'])){
			...
			foreach ($equipos as $key) {
				...
				foreach ($jugador as $jkey) {
					...
					//HACEMOS LOS MISMO CON EL SEASON
					/*
					if($jkey->ID == "79"){
						echo "<h1>SELECT r.term_taxonomy_id FROM wp_term_relationships AS r WHERE r.object_id = 29 AND (SELECT tx.taxonomy FROM wp_term_taxonomy AS tx WHERE tx.term_id = r.term_taxonomy_id) = 'sp_season'</h1>";

						echo "<h1>".count($season)."</h1>";
					}
					*/
			...
			foreach ($jugador as $key) {
				...
				//YA CON EL EQUIPO Y LAS METRICAS PROCEDEMOS A CREAR EL OBJETO QUE INSERTAREMOS
				/*
				$insertar = "a:";
				$insertar .= count($rAequipo) + 1;
					$insertar .= ":{";
						$insertar .= "i:";
				a:3:
				{
